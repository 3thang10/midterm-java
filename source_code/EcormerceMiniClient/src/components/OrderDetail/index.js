  ...
  return (
    <div ref={orderBoxRef} data-id={orderId} className={cx('order-box')}>
      ...
      <div className={cx('order-product-list')}>
        {itemList
          ? itemList.map((item) => {
              console.log('hihi', item.quantity);
              totalPrice += item.productByProductId.price * item.quantity;
              return (
                <div key={item.id} className={cx('order-product')}>
                  <div className={cx('product-item')}>
                    <div className={cx('product-imgBox')}>
                      <img className={cx('product_img')} src={require(`../../assets/images/${item.productByProductId.image}`)} alt="" />
                    </div>
                    <div className={cx('product-info')}>
                      <span className={cx('product-name')}>{item.productByProductId.title}</span>
                      <span className={cx('product-quantity')}>X{item.quantity}</span>
                      <span className={cx('product-price')}>${item.productByProductId.price}</span>
                    </div>
                  </div>
                  <span className={cx('product-total')}>${priceFormat(item.productByProductId.price * item.quantity)}</span>
                </div>
              );
            })
          : []}
