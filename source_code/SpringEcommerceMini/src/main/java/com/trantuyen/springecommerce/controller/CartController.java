...
public class CartController {
    ...
    public ResponseEntity<String> addProductToCart(@RequestBody CartModel.AddProductToCart addProductToCart, @AuthenticationPrincipal Customer customer) {
        List<Cart> carts = cartRepo.findCartsByCustomerByCustomerId(customer.getId());
        Optional<Cart> optCart = carts.stream().filter(c -> c.getProductByProductId().getId() == addProductToCart.getProductId())
                .findFirst();
    ...
    @RequestMapping(value = "/carts/delete/{id}", method = RequestMethod.POST)
    public ResponseEntity<String> deleteCartById(@PathVariable Long id, @AuthenticationPrincipal Customer customer){
        // check
